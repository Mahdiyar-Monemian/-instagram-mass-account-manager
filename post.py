...
SETTING_TXT = os.path.join(DIRNAME, "post_setting.txt")
COOKIES_FOLDER = os.path.join(DIRNAME, "user_cookies_android")
MOBILE_FOLDER = os.path.join(DIRNAME, "user_phones");
CAPTIONS_D_TXT = os.path.join(DIRNAME, "post_captions_D.txt")
CAPTIONS_P_TXT = os.path.join(DIRNAME, "post_captions_P.txt")
chromedriver = os.path.join(DIRNAME, "chromedriver.exe")

post_photos_D = glob.glob(f"{DIRNAME}\Posts_photo_D\*")
post_photos_P = glob.glob(f"{DIRNAME}\Posts_photo_P\*")


with open(CAPTIONS_D_TXT, 'r', encoding='utf-8') as fb:
    post_captions_D = fb.read().splitlines()

with open(CAPTIONS_P_TXT, 'r', encoding='utf-8') as fb:
    post_captions_P = fb.read().splitlines()
...
with open(SETTING_TXT, 'r') as fb:
    ...
    post_min = int(lines[7])
    post_max = int(lines[9])
...
for user in users:
    ...
    for y in range(3):
        try:
            ...
            for z in range(random.randint(post_min, post_max)):
                click_by_xpath(xpaths["android_homescreen_notnow"], 2, waituntil=False)
                click_by_xpath(xpaths["android_send_photo"], 2)
                if(user[2] == 'D'):
                    photo = random.choice(post_photos_D)
                    caption = random.choice(post_captions_D)
                else:
                    photo = random.choice(post_photos_P)
                    caption = random.choice(post_captions_P)

                sleep(3)
                pyautogui.write(photo)
                pyautogui.press('enter')
                click_by_xpath(xpaths["android_send_photo_next"], 3)
                send_by_xpath(xpaths["android_send_photo_caption"], caption, 3)
                click_by_xpath(xpaths["android_send_photo_share"], 3)
                t = random.uniform(wait_min, wait_max)
                print(f"Sleeping for {t} seconds")
                sleep(t)

            print(f"successfully posted this photo:\n{photo}\nwith this caption:\n{caption}\nfor {user[0]}")
            break
